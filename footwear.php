...
<form method="POST" action="http://localhost/shopmaniac/check.php"  name="login123">
  ...
  <?php
  echo "<input type=\"hidden\" name=\"flag\" value=\"10002\" />";
  ?>
...
$aaa=mysql_connect("localhost","root","system");
mysql_select_db("eshop",$aaa);
$qry=mysql_query("select count(*) from products where dept='footwear'");
while($row = mysql_fetch_array($qry))
{
	$count=$row['count(*)'];
}
...
$q=0;
$i=0;		
	$fetch=mysql_query("select * from products where dept='footwear'");
