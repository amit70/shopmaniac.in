...
<form method="POST" action="http://localhost/shopmaniac/check.php"  name="login123">
  ...
  <td><?php 
  echo "<input type=\"text\" name=\"Username\" id=\"email123\">";
  ?></td>
  </tr>
  <tr>
...
echo "<p style=\"position:absolute; left:50%; top:-16%; font-size:80%; color:#717171; font-family:'Comic Sans MS', cursive\">Hi,".$_SESSION['uname']."SSSSS|</p>";
?>
...
$aaa=mysql_connect("localhost","root","system");
mysql_select_db("eshop",$aaa);
$qry=mysql_query("select count(*) from products where subname='casual shirts'");
while($row = mysql_fetch_array($qry))
{
	$count=$row['count(*)'];
}
