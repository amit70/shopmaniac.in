...
$qq1=spliti(",",$qq0[1]);
$c1=$c2=$c3=$c4=-1;
if(strcmp($aaa,"Choose Category")!=0)
{
	$awt=spliti("-",$aaa);
	$c1=$awt[2];
...
if(strcmp($bbb,"Choose Size")!=0)
{
	$awt=spliti("-",$bbb);
	$c2=$awt[2];
}
if(strcmp($ccc,"Choose Brand")!=0)
{
	$awt=spliti("-",$ccc);
	$c3=$awt[2];
}
if(strcmp($ddd,"Choose Color")!=0)
{
	$awt=spliti("-",$ddd);
	$c4=$awt[2];
}
?>
<form action="http://localhost/shopmaniac/searchresponse.php" method="POST" name="filter" id="filter">
...
$id1=array();
$fl=0;
for($v=0;$v<count($qq1);$v++)
	{
   $qry1111="select * from products where pid=".(int)$qq1[$v];
...
for($i=0;$i<count($misc);$i++)
	{
		if($c1==$i)
		{
	echo "<option value=\"$misc[$i],$idstr-$idstr1\" selected=\"selected\">";
		}
		else
		{
	echo "<option value=\"$misc[$i],$idstr-$idstr1\">";
		}
...
for($v=0;$v<count($ids);$v++)
	{
   $qry1111="select * from products where pid=".(int)$ids[$v];
$fetch1111=mysql_query($qry1111);
while($row1111 = mysql_fetch_array($fetch1111))
{
		$size=$row1111['size'];
...
if(strcmp($aaa,"Choose Category")!=0)
{
	...
	$qry1="select * from products where category='$qq[0]' and (";
	$qry2="select count(*) from products where category='$qq[0]' and (";														   
	for($i=1;$i<count($qq);$i++)
	{
		if($i!=count($qq)-1)
		{
			$qry1=$qry1."pid=$qq[$i] or ";
			$qry2=$qry2."pid=$qq[$i] or ";
		}
		else
		{
			$qry1=$qry1."pid=$qq[$i])";
...
else if(strcmp($bbb,"Choose Size")!=0)
{
	$qq0=spliti("-",$bbb);
	$rr=spliti(",",$qq0[0]);
...
$qq1=spliti(",",$qq0[1]);
	$qry1="select * from products where size='$rr[0]' and (";
	$qry2="select count(*) from products where size='$rr[0]' and (";														   
	for($i=1;$i<count($rr);$i++)
	{
		if($i!=count($rr)-1)
		{
			$qry1=$qry1."pid=$rr[$i] or ";
			$qry2=$qry2."pid=$rr[$i] or ";
		}
		else
		{
			$qry1=$qry1."pid=$rr[$i])";
...
else if(strcmp($ccc,"Choose Brand")!=0)
{
	$ss=spliti(",",$ccc);
	$qry1="select * from products where brand='$ss[1]'";
		$qry2="select count(*) from products where brand='$ss[1]'";

}
else if(strcmp($ddd,"Choose Color")!=0)
{
	$tt=spliti(",",$ddd);
	$qry1="select * from products where color='$tt[1]'";
		$qry2="select count(*) from products where color='$tt[1]'";
...
else if(strcmp($_POST['range'],"Price Range")!=0)
{
	...
	$range=$_POST['range'];
	$arr=spliti("-",$range);
	$qry1="select * from products where  price BETWEEN $arr[0] AND $arr[1]";
	$qry2="select count(*) from products where price BETWEEN $arr[0] AND $arr[1]";
	$test=30;
}
else if(strcmp($_POST['category'],"Choose Category")!=0 && strcmp($_POST['size1'],"Choose Size")!=0)
{
	...
	$qq2=spliti(",",$bbb);
		$qq0=spliti("-",$aaa);
$qq1=spliti(",",$qq0[1]);
$qry1="select * from products where category='$qq[0]' and size='$qq2[0]' and(";
for($i=1;$i<count($qq);$i++)
	{
		if($i!=count($qq)-1)
		{
...
echo "<p style=\"position:absolute; left:2%; top:60%; font-family:'Comic Sans MS', cursive; font-weight:bold; font-size:150%;\">MEN'S FORMAL SHIRTS ($count) $test</p><hr style=\"position:absolute; left:2%; top:70%; width:96%;\"/>";
?>
